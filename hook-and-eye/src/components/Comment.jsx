...
const Comments = ({ post_id }) => {
    ...
    // Fetch comments when component mounts or post_id changes
    useEffect(() => {
        if (post_id) {
            fetchComments();
        }
    ...
    return (
        <div>
            <Typography variant="h5" className="p-2 pl-4">
                Comments ({comments.length})
            </Typography>
            
            {/* Display Comments */}
            <div className="flex flex-col gap-3 p-4 mb-2 max-h-96 overflow-y-auto">
                {loading ? (
                    ...
                    comments.map((comment) => (
                        <div key={comment.id} className="bg-primary p-3 rounded-lg shadow-sm">
                            <div className="flex justify-between items-start mb-1">
                                <Typography className="font-semibold text-text text-base">
            ...
            {/* Comment Input */}
            <div className="flex w-full flex-row items-center gap-2 rounded-[99px] border border-gray-900/10 bg-gray-900/5 p-2 pl-4">
                <div className="relative grid h-full w-full min-w-[200px]">
                    <textarea 
                        ...
                        disabled={submitting}
                        className="peer h-full min-h-full w-full resize-y rounded-[7px] !border-0 border-blue-gray-200 border-t-transparent bg-transparent px-3 py-2.5 font-sans text-base font-semibold text-blue-gray-700 outline outline-0 transition-all placeholder:text-blue-gray-300 placeholder-shown:border placeholder-shown:border-blue-gray-200 placeholder-shown:border-t-blue-gray-200 focus:border-2 focus:border-gray-900 focus:border-transparent focus:border-t-transparent focus:outline-0 disabled:resize-none disabled:border-0 disabled:bg-blue-gray-50"
                    />
                ...
                <div>
                    <button
                        className="relative h-10 max-h-[40px] w-10 max-w-[40px] select-none rounded-full text-center align-middle font-sans text-md font-medium uppercase text-gray-900 transition-all hover:bg-gray-900/10 active:bg-gray-900/20 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                        type="button"
